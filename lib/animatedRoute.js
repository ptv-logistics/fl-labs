...
var buildD3Animations = function (alts, replaySpeed) {
    var x;

    if (!map.xxx) {
        x = new L.SvgLayer().addTo(map);
        map.xxx = x;
    }
    else
        x = map.xxx;
    ...
    for (var i = 0; i < alts.length; i++) {
        buildD3Animation(alts[i], i, svg, replaySpeed);
    }
}

var buildD3Animation = function (route, index, svg, replaySpeed) {
    ...
    function binaryIndexOf(segments, searchElement) {
        ...
        while (minIndex <= maxIndex) {
            ...
            else if (currentElement >= searchElement && (currentIndex > 1 && segments[currentIndex - 1].accTime >= searchElement)) {
